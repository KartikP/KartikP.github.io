---
title: 'Max Interval Burst Detection (Demo)'
date: 2023-09-18
permalink: /posts/MaxIntervalPy
tags:
  - projects
  - algorithm
  - neuroscience
...
::: {.cell .markdown id="S_NJHeZkJzSR"}
## Import libraries
:::

::: {.cell .code id="5IOnMdVrZL2T"}
``` python
import numpy as np
import pandas as pd
...
::: {.cell .markdown id="4DKgZhPbJ5Ia"}
## Load data
:::

::: {.cell .code id="VN9cSmX0ZOX7"}
``` python
spiketrain = pd.read_csv("/content/drive/MyDrive/spiketrain_example.csv")
```
:::

::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":204}" id="RuIAqHPrh1H5" outputId="82daf668-9bf7-4e30-ad52-6fda0dba718c"}
``` python
spiketrain.head()
```
...
::: {.cell .markdown id="j-h3vWwaJ9WN"}
## Visualize data

Data are spike times of action potentials detected by a single
electrode.
:::

::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":713}" id="GCpyhMQXiDu0" outputId="f2e14cb1-1d24-4a33-8d50-36c28fbc2aee"}
``` python
...
::: {.output .display_data}
![](MaxIntervalPy Images/f34ee63e45c5b54c5ad2bf50ac2a0127c49ff687.png)
:::
:::

::: {.cell .markdown id="-_TAIKCtKHGR"}
## Max Interval Burst Detection

A Python- based Max Interval (MI) burst detection algorithm was created
to identify individual bursts. MI is a fixed threshold-based method for
identifying bursts that uses five fixed threshold parameters (maximum
ISI at start of the burst, maximum ISI in burst, minimum burst duration,
minimum IBI, and minimum number of spikes within burst) to identify,
merge, and exclude potential bursts. The values for these parameters are
chosen a priori.
:::

::: {.cell .code id="_DXeDo_pZOf-"}
``` python
...
::: {.cell .markdown id="9XyT46WFKogQ"}
## Detect Bursts
:::

::: {.cell .code id="p_rnVONOkKIt"}
``` python
allBurstData, _ = maxInterval(spiketrain["Time (s)"].values)
```
:::

::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\"}" id="hCCixZZ2ke8u" outputId="12de7f90-026a-4a0a-978f-87368948c932"}
``` python
...
::: {.cell .markdown id="RRv-KFXkKr96"}
## Plot the results
:::

::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":713}" id="_fTpdbz0kq9T" outputId="f79748c8-bf1e-4fef-aaba-da957373aef2"}
``` python
f = plt.figure(figsize=(20, 8), constrained_layout=False)
grid = f.add_gridspec(2, 3)
plt.subplot(grid[0,0:3])
...
::: {.output .display_data}
![](MaxIntervalPy Images/08e6f252c01818c82ea76e98653711af26736985.png)
:::
:::

::: {.cell .markdown id="ku7H_tJmlLK1"}
## Calculate some basic burst features
:::

::: {.cell .code id="5TLOA8zHlJcW"}
``` python
...
::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":466}" id="yegfrJ-hl-SS" outputId="70090bf3-fd32-436c-c0fd-7fbb0efd63a6"}
``` python
...
::: {.output .display_data}
![](MaxIntervalPy Images/fe84cc43e6ecac9e74a8ef3e04af95f9687fa63c.png)
:::
:::

::: {.cell .code colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":466}" id="jf8FCtIFmKTY" outputId="ab07a2d0-7e9b-4d24-face-9632bc78921f"}
``` python
...
::: {.output .display_data}
![](MaxIntervalPy Images/b99f07055cb72e8d4c897112707a1657f1c17a2f.png)
:::
:::
